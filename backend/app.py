...
from services.rate_limiting import init_rate_limiting, limiter
...
def create_app(config_name='development'):
    ...
    # Rate Limiting (C2.1 — quotas utilisateurs)
    init_rate_limiting(app)

    # Initialisation du DataLoader (PostgreSQL)
    try:
        initialize_data_loader()
        logger.info("DataLoader initialisé avec PostgreSQL")
    except Exception as e:
        logger.error(f"Erreur lors de l'initialisation du DataLoader: {e}")
        logger.warning("L'application démarre sans données - vérifiez PostgreSQL")
    ...
    def index():
        ...
        return jsonify({
            'message': 'API Urban Data Explorer — Paris',
            ...
            'endpoints': {
                ...
                'auth':            '/api/auth/login | /api/auth/me | /api/auth/verify'
            }
        })
    ...
    def health():
        ...
        try:
            ...
            return format_error(
                'Service unhealthy — vérifiez que PostgreSQL est démarré (docker ps)',
                status=503
            )
        ...
        try:
            ...
            for arr_data in tous_arrs:
                apercu.append({
                    ...
                    'population_2022':   arr_data.get('population_2022'),
                    'revenu_median':     arr_data.get('revenu_median'),
                    'indice_accessibilite': arr_data.get('indice_accessibilite')
                })
...
if __name__ == '__main__':
    ...
    print("=" * 70)
    print("🚀 URBAN DATA EXPLORER — API Backend")
    print("=" * 70)
    ...
    print(f"🔐 Auth login   : POST http://localhost:{port}/api/auth/login")
    print("=" * 70)
